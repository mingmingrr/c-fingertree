...
instance CEquiv a => CEquiv (S.Elem a) where
  type instance Target (S.Elem a) = Node
  toTarget (S.Elem x) = toTarget x >>= \ptr ->
    node_make 1 (castPtr ptr) nullPtr nullPtr
  toSource ptr = toSource (#{ptr Node, items} ptr)

instance (CEquiv a, Target a ~ Node) => CEquiv (S.Node a) where
  type instance Target (S.Node a) = Node
...
instance (S.Sized a, CEquiv a, Target a ~ Node) => CEquiv (S.Digit a) where
  type instance Target (S.Digit a) = Digit
  toTarget n = case n of
...
instance (S.Sized a, CEquiv a, Target a ~ Node) => CEquiv (S.FingerTree a) where
  type instance Target (S.FingerTree a) = Tree
  ...
  toSource ptr = peek ptr >>= \(Tree _ typ val) -> case typ of
    EmptyT -> pure S.EmptyT
    SingleT -> S.Single <$> toSource (castPtr val :: Ptr Node)
...
equalTree :: S.Seq Int -> Ptr Tree -> IO ()
